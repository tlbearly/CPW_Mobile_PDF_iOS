...
class WayPt: NSObject, NSCoding {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        guard let imageName = aDecoder.decodeObject(forKey: "imageName") as? String else { os_log("Unable to decode the way point image name.", log: OSLog.default, type: .debug)
            return nil
        }
        guard let desc = aDecoder.decodeObject(forKey: "desc") as? String else { os_log("Unable to decode the way point description.", log: OSLog.default, type: .debug)
            return nil
        }
        guard let dateAdded = aDecoder.decodeObject(forKey: "dateAdded") as? String else { os_log("Unable to decode the way point date added.", log: OSLog.default, type: .debug)
            return nil
        }
